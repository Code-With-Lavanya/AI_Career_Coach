...
def get_llm():
    api_key = os.getenv("MISTRAL_API_KEY") or st.session_state.get("api_key", "")
    return ChatMistralAI(api_key=api_key, temperature=0)
...
with st.sidebar:
    ...
    st.markdown('<div class="nav-section">API Configuration</div>', unsafe_allow_html=True)
    api_val = st.text_input("Mistral API Key", type="password",
                            placeholder="sk-...",
                            value=st.session_state.get("api_key", ""))
    if api_val:
        st.session_state.api_key = api_val
        os.environ["MISTRAL_API_KEY"] = api_val
    ...
    if has_resume and has_jd:
        ...
        with col_btn:
            if st.button("🚀 Build Knowledge Base", use_container_width=True):
                if not (os.getenv("MISTRAL_API_KEY") or st.session_state.get("api_key")):
